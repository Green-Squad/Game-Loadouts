            ...
            if (count($attachments) >= $weapon -> min_attachments && count($attachments) <= $weapon -> max_attachments) {
                ...
                if ($existingLoadout) {
                    ...
                    if ($this -> userHasLoadout($user -> loadouts, $existingLoadout -> id)) {
                        ...
                        $user -> save();
                        return Redirect::route('showLoadout', array($game -> id, $weapon -> name, $existingLoadout -> id)) -> with(array(
                ...
                return Redirect::route('showLoadout', array($game -> id, $weapon -> name, $loadout -> id)) -> with(array(
